...
checkRescInput (*Res) {
# check whether *Res is a resource
  *q1 = select count(RESC_ID) where RESC_NAME = '*Res';
  foreach (*r1 in *q1) {*n = *r1.RESC_ID;}
  if (*n == "0") {
    writeLine("stdout", "Resource *Res is not defined");
    fail;
  }
}

checkUserInput (*User) {
# check whether *User is a user
# fail if not a user
  *Q = select count(USER_ID) where USER_NAME = '*User';
  foreach (*R in *Q) {*Result = *R.USER_ID;}
  if(*Result == "0" ) {
    writeLine("stdout","Input *User is not a user");
    fail;
  }
}
...
# acsendAccess.r
  ...
  *eventOutcome = jsonEncode(*eventOutcomeJson);
  *msg='
    {
      "messages" : [ {
        "operation" : "create",
...
# acrelatedEvent.r
  *relationshipType = jsonEncode(*relationshipTypeJson);
  *relationshipSubType = jsonEncode(*relationshipSubTypeJson);
  *msg='
  {
    "messages" : [ {
        "operation" : "create",
